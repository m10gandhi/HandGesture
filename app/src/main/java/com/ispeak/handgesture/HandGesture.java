		...
		if ((detectIsHand(img))) {
			...
			if (defectIdAfter.size() > 0) {
				...
				for (int j = startId; ; j++)
				{
					...
					Imgproc.circle(img, curDefectPoint, 2, new Scalar(0, 0, 255), -5);
			...
			for (int fid = 0; fid < finTipsTemp.size(); )
			{
				...
				Imgproc.line(img, inCircle, curFinPoint, new Scalar(24, 77, 9), 2);
				Imgproc.circle(img, curFinPoint, 2, Scalar.all(0), -5);
				
				Imgproc.putText(img, Integer.toString(count), new Point(curFinPoint.x - 10,
